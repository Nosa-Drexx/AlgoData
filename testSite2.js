...
function connect(id, depth){
        ...
        connectionArray.forEach((connectionelem)=>{
            list.forEach((elem)=>{
                if (connectionelem == elem.id){
                    elem.connections.forEach((connectedelem)=>{ //avoid repetition of ids
    ...
    connectionArray.forEach((elem)=>{
        list.forEach((elem2)=>{
            if (elem === elem2.id){
                getMostPopularjob.forEach((elemPopular)=>{// increase job of each scores for people with same jobs
                    if(elemPopular.job === elem2.title){
                        elemPopular.score = elemPopular.score + 1;
                        if(elemPopular.score > scoreofMostPopularjob){ // get most popular job by its score
                            scoreofMostPopularjob = elemPopular.score;
                            mostPopularjob = elemPopular.job;
                        }
                    }
                })
                if(!ArrayofAllJobs.includes(elem2.title)){ // Avoid repition of people with same jobs
